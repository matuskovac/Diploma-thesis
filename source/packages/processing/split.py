...
def temp(df, all_usernames, new_user_usernames, y_column):
    ...
    for user, patterns_id in unique_patterns_id.iteritems():
        if(user in new_user_usernames):
            pattern_ids_to_val += list(
                patterns_id[:round(len(patterns_id)*0.5)])
            pattern_ids_to_test += list(
                patterns_id[round(len(patterns_id)*0.5):])
        else:
            pattern_ids_to_train += list(
                patterns_id[:round(len(patterns_id)*0.7)])
            pattern_ids_to_val += list(
                patterns_id[round(len(patterns_id)*0.7):round(len(patterns_id)*0.85)])
            pattern_ids_to_test += list(
                patterns_id[round(len(patterns_id)*0.85):])

    mask = [True if x['id']
            in pattern_ids_to_train else False for i, x in df.iterrows()]
    df_part_train = df[mask]

    mask = [True if x['id']
            in pattern_ids_to_val else False for i, x in df.iterrows()]
    df_part_val = df[mask]

    mask = [True if x['id']
            in pattern_ids_to_test else False for i, x in df.iterrows()]
...
def split_to_train_val_test_raw(df, y_column):
    ...
    df_raw_train, df_raw_val, df_raw_test = temp(
        df_shuffled.loc[df_shuffled['scenario'] == 'scenario_show_simple'], all_usernames, new_user_usernames, y_column)
    df_raw_train2, df_raw_val2, df_raw_test2 = temp(
        df_shuffled.loc[df_shuffled['scenario'] == 'scenario_show_complex'], all_usernames, new_user_usernames, y_column)
    ...
    df_raw_train = df_raw_train.sample(
        frac=1, random_state=20).reset_index(drop=True)
    df_raw_val = df_raw_val.sample(
        frac=1, random_state=20).reset_index(drop=True)
    df_raw_test = df_raw_test.sample(
        frac=1, random_state=20).reset_index(drop=True)
