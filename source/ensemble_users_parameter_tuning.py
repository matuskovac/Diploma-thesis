...
iterables = [all_models, all_ensemble_based_on_users, all_ensemble_based_on_segments,
             all_functions_to_ensemble_users, all_kind_of_patten, all_predict_based_on_whole_pattern]
...
for count_of_owners in range(1, 5):
    ...
    for model_to_use, ensemble_based_on_users, ensemble_based_on_segments,  function_to_ensemble_users_raw, kind_of_patten, predict_based_on_whole_pattern in itertools.product(*iterables):
        ...
        rows.append([model_to_use, ensemble_based_on_users, count_of_owners, predict_based_on_whole_pattern, kind_of_patten,
                     ensemble_based_on_segments, function_to_ensemble_users_raw, train_eer, val_eer, test_eer])
        print(len(rows))


df_tuning = pd.DataFrame(rows, columns=[
    "model", "ensemble_based_on_users", "count_of_owners", "predict_based_on_whole_pattern", "kind_of_patten", "ensemble_based_on_segments", "function_to_users", "train_eer", "val_eer", "test_eer"])

df_tuning.to_csv("../results/cont_ensemble_users.csv",
